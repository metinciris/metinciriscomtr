...
import { Copy, Check, Plus, Trash2, Microscope, FileText, RotateCcw, LayoutGrid, Info } from 'lucide-react';
...
export function TiroidPapillerKarsinom() {
    ...
    useEffect(() => {
        ...
        currentLines.forEach((line) => {
            ...
            if (!prevLines.includes(line)) {
                ...
                // Schedule cleanup for this specific line after 3 seconds
                setTimeout(() => {
    ...
    const deleteTumor = (id: number) => {
        if (tumors.length > 1) {
            setTumors(tumors.filter(t => t.id !== id));
    ...
    const getToggleClass = (key: string, isActive: boolean, isBad: boolean = false) => {
        ...
        if (isActive) {
            if (isBad) {
                base += 'bg-red-500 border-red-600 text-white shadow-sm';
            } else {
    ...
    return (
        <PageContainer>
            ...
            <style dangerouslySetInnerHTML={{ __html: `
                ...
                .animate-line-flash {
                    animation: line-flash 2.5s ease-out forwards;
                    padding-left: 8px;
                    margin-left: -8px;
                    border-radius: 4px;
                }
                        ...
                        <div ref={controlsPanelRef} className="lg:col-span-7 space-y-5">
                            ...
                            <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4">
                                <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
                                    ...
                                    <button
                                        onClick={addTumor}
                                        className="flex items-center gap-1 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold shadow-sm transition-all"
                                    >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {tumors.map((tumor, idx) => (
                                        <div 
                                            key={tumor.id} 
                                            className={`p-3.5 border rounded-xl bg-slate-50/50 shadow-sm flex flex-col justify-between transition-all ${
                                                changedFields[`tumor-add-${tumor.id}`] ? 'border-indigo-500 bg-indigo-50/10' : 'border-slate-200'
                                            }`}
                                        >
                                            {/* Card Header */}
                                            ...
                                            <div className="space-y-3 flex-1">
                                                        ...
                                                        <div className="flex gap-1">
                                                            {['Yoktur', 'vardır', 'Kısmen vardır'].map(opt => (
                                                                <button
                                                                    ...
                                                                    className={getToggleClass(`tumor-${tumor.id}-encapsulation`, tumor.encapsulation === opt)}
                                                                >
                                                        ...
                                                        <div className="flex gap-1">
                                                            {['yoktur', 'VARDIR', 'Kuşkulu VARDIR'].map(opt => (
                                                                <button
                                                                    ...
                                                                    className={getToggleClass(`tumor-${tumor.id}-lvInvasion`, tumor.lvInvasion === opt, opt !== 'yoktur')}
                                                                >
                                                ...
                                                {/* LVI Types (Only shown if LVI is VARDIR) */}
                                                {tumor.lvInvasion === 'VARDIR' && (
                                                    <div className="bg-white border border-slate-200 p-2 rounded-lg flex items-center justify-around gap-2">
                                                        <label className="flex items-center gap-1.5 cursor-pointer text-xs font-bold text-slate-700">
                                                            <input
                                                                type="checkbox"
                                                                checked={tumor.lymphaticInvasion}
                                                                onChange={(e) => updateTumor(tumor.id, 'lymphaticInvasion', e.target.checked)}
                                                                className="rounded text-indigo-600 w-4 h-4 cursor-pointer focus:ring-0"
                                                            />
                                                            Lenfatik
                                                        </label>
                                                        <label className="flex items-center gap-1.5 cursor-pointer text-xs font-bold text-slate-700">
                                                ...
                                                <div className="bg-white border border-slate-200 p-2 rounded-lg space-y-1">
                                                    ...
                                                    {CAPSULE_ET_OPTIONS.map(opt => {
                                                        ...
                                                        return (
                                                            <label key={opt} className="flex items-center gap-2 cursor-pointer text-xs font-bold text-slate-600 hover:text-slate-800">
                                                                <input
                                                                    type="checkbox"
                                                                    checked={isChecked}
                                                                    onChange={(e) => {
                                                                    ...
                                                                    className="rounded text-indigo-600 w-3.5 h-3.5 cursor-pointer focus:ring-0"
                                                                />
                                                ...
                                                {/* Strap Muscle (Only if ET is Var) */}
                                                        ...
                                                        <div className="flex gap-1">
                                                            {STRAP_OPTIONS.map(opt => (
                                                                <button
                                                                    ...
                                                                    className={getToggleClass(`tumor-${tumor.id}-strapMuscle`, tumor.strapMuscle === opt, opt.includes('VARDIR'))}
                                                                >
                                                            ...
                                                            <div className="flex gap-1">
                                                                {['Cerrahi sınırlarda tümör yoktur', 'Cerrahi sınırlarda tümör VARDIR'].map(opt => (
                                                                    <button
                                                                        ...
                                                                        className={getToggleClass(`tumor-${tumor.id}-marginsTumorPresent`, (opt === 'Cerrahi sınırlarda tümör VARDIR') === tumor.marginsTumorPresent, opt === 'Cerrahi sınırlarda tümör VARDIR')}
                                                                    >
                                ...
                                {lymphNodes.length === 0 ? (
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {lymphNodes.map((ln, idx) => (
                                            <div 
                                                key={ln.id} 
                                                className={`p-3.5 border rounded-xl bg-slate-50/50 shadow-sm space-y-3 flex flex-col justify-between transition-all ${
                                                    changedFields[`ln-add-${ln.id}`] ? 'border-indigo-500 bg-indigo-50/10' : 'border-slate-200'
                                                }`}
                                            >
                                                {/* LN Header */}
                                                ...
                                                <div className="space-y-3 flex-1">
                                                    ...
                                                    {ln.metastatic > 0 && (
                                                        <div className="bg-white border border-slate-200/80 p-2.5 rounded-lg space-y-3">
                                                            <div>
