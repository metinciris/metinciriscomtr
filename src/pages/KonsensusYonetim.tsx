...
export function KonsensusYonetim() {
  ...
  const fetchMeetings = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from('meetings')
      .select('id, title, organizer, date, time, duration, description, poster_url, zoom_link, zoom_id, zoom_password, created_at')
  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      if (!alive) return;
      const validAdmin = checkAdmin(session);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-8">
        <div className="flex items-center gap-3 text-gray-600 font-black">
          ...
          <span>Yönetim Paneli Doğrulanıyor…</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-gradient-to-r from-slate-800 to-indigo-900 text-white p-8 sm:p-10 rounded-3xl shadow-lg relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full blur-3xl -mr-32 -mt-32" />
          <div className="relative z-10 text-center">
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-black tracking-tight flex items-center justify-center gap-3">
              <Lock className="w-8 h-8 text-blue-400" />
              Konsensus Yönetim Paneli
            </h1>
            <p className="mt-3 text-white/90 text-sm sm:text-base font-semibold">
              Toplantı ekleme, düzenleme, silme ve tam Zoom/afiş erişim alanıdır.
            </p>
          </div>
        </div>
      ...
      <RelatedPages
        pages={[
          {
            title: "Konsensüs Toplantıları",
            subtitle: "Halka açık duyuru ve katılım takvimi",
            page: "konsensus",
            color: "bg-blue-600"
          },
          {
