...
// OpenSeadragon ve GeoTIFF CDN'den yüklenecek
declare global {
  interface Window {
    OpenSeadragon: any;
    GeoTIFF: any;
...
export function SvsReader() {
  ...
  const initViewer = useCallback(async (file: File) => {
    ...
    try {
      ...
      const bits = Array.isArray(fd.BitsPerSample)
        ? fd.BitsPerSample[0]
        : fd.BitsPerSample;

      console.log('SVS info:', {
        width,
        height,
        samplesPerPixel,
        photometric,
      ...
      // Çok büyük slide'lar için basit bir limit (browser güvenliği için)
      const MAX_PIXELS = 16000 * 16000; // ~256M pixel
      if (width * height > MAX_PIXELS) {
        throw new Error(
          `Görüntü çok büyük (${width}×${height}). Tarayıcı belleği için fazla büyük olabilir.`
        );
      }

      // Şu an için sadece gri (0/1) ve RGB (2) destekleyelim
      if (photometric !== 0 && photometric !== 1 && photometric !== 2) {
        throw new Error(
          `Şu anda PhotometricInterpretation=${photometric} desteklenmiyor. (Sadece 0, 1, 2)`
        );
      }
      ...
      if (samplesPerPixel >= 3) {
        ...
        if (bits === 8) {
          ...
          for (let i = 0; i < pixelCount; i++) {
            ...
            rgba[dst + 3] =
              samplesPerPixel >= 4 ? data[src + 3] : 255; // A
          }
        } else if (bits === 16) {
          ...
          throw new Error(
            `Desteklenmeyen bit derinliği (RGB): ${bits}-bit`
          );
        }
        ...
        if (bits === 8) {
          ...
          throw new Error(
            `Desteklenmeyen bit derinliği (gri): ${bits}-bit`
          );
        }
      } else {
        throw new Error(
      ...
      // Data URL'e çevirip viewer'a ver
  ...
  return (
    <PageContainer>
      <div id="svs-reader" style={styles.page}>
          ...
          <div style={styles.headerTitle}>
            ...
            <div>
              ...
              <p style={styles.headerDesc}>
                Aperio SVS ve TIFF formatındaki dijital patoloji
                slaytlarını tarayıcınızda görüntüleyin. Dosyalar
                yalnızca tarayıcınızda işlenir, sunucuya yüklenmez.
              </p>
              {!scriptsLoaded && (
          ...
          {isLoading && (
            <div style={styles.loadingState}>
              <Loader2
                ...
                style={{ animation: 'spin 1s linear infinite' }}
              />
              <div style={styles.loadingText}>Dosya işleniyor...</div>
              <div style={styles.loadingSubtext}>
                Büyük dosyalar için bu işlem biraz sürebilir.
              </div>
            </div>
          )}
        ...
        {/* Alt bilgi kartları – resim yokken göster */}
        {!imageLoaded && (
          <div style={styles.infoGrid}>
            <div style={styles.infoCard}>
              ...
              <p style={styles.infoText}>
                Aperio SVS, BigTIFF, NDPI ve standart TIFF formatları
                desteklenmektedir. Dosyalar doğrudan tarayıcınızda
                işlenir.
              </p>
            </div>
            <div style={styles.infoCard}>
              ...
              <p style={styles.infoText}>
                Fare tekerleği ile yakınlaştırın, sürükleyerek
                kaydırın. Çift tıklama ile hızlı zoom. Sağ üstteki
                kontrolleri kullanarak görünümü sıfırlayabilir veya
                tam ekrana geçebilirsiniz.
              </p>
            </div>
            <div style={styles.infoCard}>
              <div style={styles.infoTitle}>
              ...
              <p style={styles.infoText}>
                Tüm işlemler tarayıcınızda gerçekleşir. Dosyalarınız
                sunucuya yüklenmez veya kaydedilmez. Büyük slaytlar
                için yeterli RAM gerektiğini unutmayın.
              </p>
            </div>
          </div>
        )}

        {/* Spin animasyonu için basit keyframes */}
        <style>
          {`
          @keyframes spin {
